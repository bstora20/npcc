...
static inline uintptr_t getRandomRollback(uintptr_t rollback) {
    ...
    in = (((in + 1) & -rollback) | (in & ~-rollback)) % BUFFER_SIZE;  // Roll back if rollback is zero
    return num;
}
...
struct Cell* poss_change_cell(struct Cell* cell1, struct Cell* cell2, uint64_t change){
    ...
    cell1->energy = cell1->energy * !change + cell2->lineage * change;
    for (unsigned int i = 0;i<POND_DEPTH_SYSWORDS;i++)
        cell1->genome[i] = cell1->genome[i]* !change + cell2->genome[i] * change;
...
//                        shiftPtr=(shiftPtr+4)+(shiftPtr+4>=SYSWORD_BITS)*(-shiftPtr-4);
		...
		if ((outputBuf[0] & 0xff) != 0xff) {
            ...
            /*
            int access_var = accessAllowed(tmpptr,reg,0,tmpptr->energy!=0);
		...
		*/
        ...
        (tmpptr)&& (tmpptr = poss_change_cell(tmpptr,getNeighbor(x,y,facing),((outputBuf[0] & 0xff) != 0xff)));
        int access_var = tmpptr && accessAllowed(tmpptr,reg,0,(tmpptr->energy) && (tmpptr->energy!=0));
        statCounters.viableCellsReplaced = tmpptr && (statCounters.viableCellsReplaced * !((outputBuf[0] & 0xff) != 0xff) +(statCounters.viableCellsReplaced +(tmpptr->generation>2) * (tmpptr->energy&&access_var) * ((outputBuf[0] & 0xff) != 0xff)));
        cellIdCounter = tmpptr && (cellIdCounter * !((outputBuf[0] & 0xff) != 0xff) +(cellIdCounter * !(tmpptr->energy&&access_var) + (tmpptr->energy&&access_var) * (cellIdCounter+1) * ((outputBuf[0] & 0xff) != 0xff)));
        tmpptr && (tmpptr->ID = (tmpptr->ID * !((outputBuf[0] & 0xff) != 0xff) +(tmpptr->ID * !(tmpptr->energy&&access_var) + (tmpptr->energy&&access_var) * (cellIdCounter) * ((outputBuf[0] & 0xff) != 0xff))));
        tmpptr && (tmpptr->parentID = tmpptr->parentID * !((outputBuf[0] & 0xff) != 0xff) +(tmpptr->parentID * !(tmpptr->energy&&access_var) + (tmpptr->energy&&access_var) * pptr->ID * ((outputBuf[0] & 0xff) != 0xff)));
        tmpptr && (tmpptr->lineage = tmpptr->lineage * !((outputBuf[0] & 0xff) != 0xff) + (tmpptr->lineage * !(tmpptr->energy&&access_var) + (tmpptr->energy&&access_var) * pptr->lineage * ((outputBuf[0] & 0xff) != 0xff)));
        tmpptr && (tmpptr->generation = tmpptr->generation * !((outputBuf[0] & 0xff) != 0xff) + (tmpptr->generation * (!(tmpptr->energy&&access_var)) + (tmpptr->energy&&access_var) * (pptr->generation+1) * ((outputBuf[0] & 0xff) != 0xff)));
        for(i=0;i<POND_DEPTH_SYSWORDS;++i)
            tmpptr && (tmpptr->genome[i] = tmpptr->genome[i] * !((outputBuf[0] & 0xff) != 0xff) +(tmpptr->genome[i] * !(tmpptr->energy&&access_var) + (tmpptr->energy&&access_var) * outputBuf[i] * ((outputBuf[0] & 0xff) != 0xff)));
