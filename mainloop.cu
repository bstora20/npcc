__global__ static void doReport(uintptr_t clock){
    ...
    cuPrintf("%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu",
        (uint64_t)clock,
...
int cudaMain()
{
    ...
    for (n = 0; n < 1000000; n++){
        for (m = 0 ; m < REPORT_FREQUENCYl m++){
            execute<<1,1>>(*d_threadNo,  *d_x,  *d_y,  *d_i,  *d_clock, *d_outputBuf,  *d_currentWord,  *d_wordPtr,  *d_shiftPtr, 
